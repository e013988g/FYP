...
class COLinearRegression():
    def getRecentDatabaseData(self):
        ...
        cursor = conn.cursor()
        sql_text = "SELECT TOP 200 ReadingPPM, DateRegistered FROM CO_Readings WHERE DATEPART(dd,DateRegistered) = DATEPART(dd, GETDATE()) AND DateRegistered > DATEADD(HOUR, -1, GETDATE()) ORDER BY DateRegistered ASC"
