...
class CO2LinearRegression():
    def getRecentDatabaseData(self):
        ...
        cursor = conn.cursor()
        sql_text = "SELECT TOP 300 ReadingPPM, DateRegistered FROM CO2_Readings WHERE DATEPART(dd,DateRegistered) = DATEPART(dd, GETDATE() - 7) AND DATEPART(hh, DateRegistered) = (DATEPART(hh, GETDATE()) + 2) ORDER BY DateRegistered ASC"
