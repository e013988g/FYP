...
def getRecentDatabaseData():
    ...
    cursor = conn.cursor()
    sql_text = "SELECT TOP 400 ReadingPPM, DateRegistered FROM CO2_Readings WHERE DateRegistered >= DATEADD(day,-2,GETDATE()) ORDER BY DateRegistered DESC "
...
# Forecast
fc, se, conf = fitted.forecast(400, alpha=0.05)  # 95% conf
